...
- **入力形式**: クリップボードの `text/plain`（TSV/CSV）または `text/html`（`<table>`）。Excel・スプレッドシートのコピー想定
- **転置**: 行の長さが異なる表は空セルで正規化してから転置。出力は TSV（タブ区切り）で Excel 等へ貼り付け可能
- **自動コピー**: 転置結果を `navigator.clipboard.writeText` で自動コピー
